...
const Snake: FC<ISnake> = ({ fieldCells, food, setFood, currentFoodSize, fieldSize, prevFoodSize, setPrevFoodSize }) => {
  ...
  const refactorUnitsSize = () => {
    ...
    if (prevFoodSize !== 0) {
      dispatch(pauseGame());
      setFood((prev: any) => ({ ...prev, x: (prev.x / prevFoodSize) * currentFoodSize, y: (prev.y / prevFoodSize) * currentFoodSize, eaten: true }));
  ...
  return (
    <>
      {snake.map((item, index) => {
        return <SnakeCell key={index} item={item} index={index} currentFoodSize={currentFoodSize} currentDirection={currentDirection} />;
      })}
    </>
  );
};
